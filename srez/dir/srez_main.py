...
def prepare_dirs(delete_train_dir=False):
    ...
    if not tf.gfile.Exists(FLAGS.dataset) or \
        ...
        pass  # raise FileNotFoundError("Could not find folder `%s'" % (FLAGS.dataset,))
...
def _demo():
    ...
    if not tf.gfile.IsDirectory(FLAGS.checkpoint_dir):
        pass  # raise FileNotFoundError("Could not find folder `%s'" % (FLAGS.checkpoint_dir,))
    ...
    print(gene_minput + gene_moutput + gene_output + gene_var_list + disc_real_output + disc_fake_output
          + disc_var_list)
